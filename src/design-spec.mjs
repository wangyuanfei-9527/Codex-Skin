...
const STATE_KEYS = ['idle', 'running', 'waiting', 'success', 'failed'];
...
export function validateDesignSpec(spec) {
  const errors = [];
  const rootKeys = ['schemaVersion', 'name', 'summary', 'sourceImageIndex', 'palette', 'effects', 'copy', 'assets', 'pet'];
  ...
  if (exactKeys(spec.pet, petKeys, 'spec.pet', errors)) {
    if (!text(spec.pet.name, 80)) errors.push('spec.pet.name must contain 1-80 characters');
    if (!SLUG.test(spec.pet.slug ?? '') || spec.pet.slug.length > 48) errors.push('spec.pet.slug must be a lowercase kebab-case name');
    if (!text(spec.pet.concept, 300)) errors.push('spec.pet.concept must contain 1-300 characters');
    if (exactKeys(spec.pet.states, STATE_KEYS, 'spec.pet.states', errors)) {
      for (const key of STATE_KEYS) if (!text(spec.pet.states[key], 220)) errors.push(`spec.pet.states.${key} must contain 1-220 characters`);
    }
  }
  return errors;
}
