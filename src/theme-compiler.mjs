...
function cssFor(spec, { includePet = true, includeGeneratedIcons = false } = {}) {
  ...
  const c = spec.copy;
  const suggestionButtons = '#codex-skin-studio-chrome .skin-action-card';
...
#codex-skin-studio-chrome .skin-action-card[data-skin-action-index="0"]::before { background-position: 0 0 !important; }
#codex-skin-studio-chrome .skin-action-card[data-skin-action-index="1"]::before { background-position: 100% 0 !important; }
#codex-skin-studio-chrome .skin-action-card[data-skin-action-index="2"]::before { background-position: 0 100% !important; }
#codex-skin-studio-chrome .skin-action-card[data-skin-action-index="3"]::before { background-position: 100% 100% !important; }
` : '';
...
.codex-skin-home {
  ...
  background: var(--codex-skin-background) !important;
}
.codex-skin-home::before,
.codex-skin-home::after {
  content: "";
  position: absolute;
  inset: 0;
  pointer-events: none;
...
.codex-skin-home::after {
  z-index: -1;
  background:
    linear-gradient(180deg, transparent 46%, ${hexRgba(p.background, 0.24)} 72%, ${hexRgba(p.background, 0.68)} 100%),
    linear-gradient(90deg, ${hexRgba(p.background, Math.max(0.74, e.overlayOpacity + 0.42))} 0%, ${hexRgba(p.background, Math.max(0.48, e.overlayOpacity + 0.18))} 38%, ${hexRgba(p.background, 0.10)} 70%, ${hexRgba(p.background, 0.18)} 100%);
}
.codex-skin-home > div:first-child { position: relative; z-index: 0; padding-top: 15px !important; min-height: 100% !important; }
...
.codex-skin-home [class~="group/home-suggestions"] {
  visibility: hidden !important;
  pointer-events: none !important;
}
#codex-skin-studio-chrome .skin-action-deck {
  position: absolute;
  z-index: 6;
  left: clamp(28px, 3.6vw, 64px);
  right: clamp(92px, 7vw, 126px);
  top: clamp(440px, 54%, calc(100% - 320px));
  display: none;
  grid-template-columns: repeat(4, minmax(0, 1fr));
  gap: clamp(10px, 1vw, 16px);
  pointer-events: auto;
}
#codex-skin-studio-chrome.skin-home-shell .skin-action-deck { display: grid; }
:is(${suggestionButtons}) {
  ...
  padding: 76px 15px 16px !important;
  ...
  background:
    radial-gradient(circle at 50% 24%, ${hexRgba(p.surface, 0.90)}, transparent 35%),
    linear-gradient(145deg, ${hexRgba(p.surface, 0.90)}, ${hexRgba(p.surfaceAlt, 0.80)}) !important;
  color: var(--codex-skin-text) !important;
  font-family: inherit;
  font-weight: 620 !important;
  line-height: 1.45 !important;
  cursor: pointer;
  box-shadow: 0 10px 24px ${hexRgba(p.background, 0.24)}, inset 0 0 0 1px ${hexRgba(p.border, 0.54)} !important;
  backdrop-filter: blur(${Math.max(10, e.blur)}px) !important;
  transition: box-shadow .18s ease-out, background-color .18s ease-out, translate .18s ease-out !important;
}
:is(${suggestionButtons}):hover {
  translate: 0 -3px;
  box-shadow: 0 14px 30px ${hexRgba(p.accent, 0.22)}, inset 0 0 0 1px ${hexRgba(p.accent, 0.58)} !important;
}
:is(${suggestionButtons}):focus-visible {
  outline: 2px solid var(--codex-skin-accent);
  outline-offset: 3px;
}
:is(${suggestionButtons}) b {
  color: var(--codex-skin-text) !important;
  font-size: 14px !important;
  font-weight: 720 !important;
  line-height: 1.25 !important;
}
:is(${suggestionButtons}) small {
...
:root.codex-skin-studio-active .composer-surface-chrome {
  ...
  background:
    radial-gradient(circle at 88% 22%, ${hexRgba(p.surface, 0.86)}, transparent 24%),
    linear-gradient(145deg, ${hexRgba(p.surface, 0.92)}, ${hexRgba(p.surfaceAlt, 0.82)}) !important;
  box-shadow: 0 14px 34px ${hexRgba(p.background, 0.28)}, inset 0 0 0 1px ${hexRgba(p.border, 0.58)} !important;
  backdrop-filter: blur(${Math.max(12, e.blur)}px) !important;
  overflow: visible !important;
}
...
:is(.codex-skin-home .skin-project-toolbar, .codex-skin-home div:has(> .horizontal-scroll-fade-mask [class~="group/project-selector"])) {
  ...
  border: 0 !important;
  background: linear-gradient(180deg, ${hexRgba(p.surface, 0.88)}, ${hexRgba(p.surfaceAlt, 0.80)}) !important;
  color: var(--codex-skin-text) !important;
...
@media (max-width: 1120px) {
  ...
  #codex-skin-studio-chrome .skin-action-deck { right: clamp(24px, 3.6vw, 44px); }
...
@media (max-width: 900px) {
  ...
  #codex-skin-studio-chrome .skin-action-deck {
    left: 22px;
    right: 22px;
    top: clamp(330px, 42%, calc(100% - 300px));
